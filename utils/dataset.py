...
class BatchWrapper(object):
    def __init__(self, dl, mixup=False, mixup_args=(3, 3)):
...
class BatchWrapper(object):
    def __init__(self, dl, dim, mixup=False, mixup_args=(7, 7)):
        super(BatchWrapper, self).__init__()
        self.dl = dl
        self.dim = dim
        self.mixup = mixup
    ...
    def __iter__(self):
        for batch in self.dl:
            if self.mixup:
                ...
                batcher = SampleWrapper(batch, self.mixup_args)
                for inst1, inst2 in batcher:
                    batch1.append(inst1)
                    batch2.append(inst2)

                batch_mixup_lmbd1 = np.random.beta(*self.mixup_args, (len(batch), self.dim))
                batch_mixup_lmbd1 = np.where(batch_mixup_lmbd1 < 0, 0., batch_mixup_lmbd1)
                batch_mixup_lmbd1 = np.where(batch_mixup_lmbd1 > 1, 1., batch_mixup_lmbd1)
                # batch_mixup_lmbd1 = max(batch_mixup_lmbd1, 1-batch_mixup_lmbd1)
                batch_mixup_lmbd2 = 1. - batch_mixup_lmbd1
                batch_mixup_lmbd1 = torch.FloatTensor(batch_mixup_lmbd1).unsqueeze(1)  # (B, 1, D)
                batch_mixup_lmbd2 = torch.FloatTensor(batch_mixup_lmbd2).unsqueeze(1)
                yield batch1, batch_mixup_lmbd1, \
                      batch2, batch_mixup_lmbd2
            else:
...
class SampleWrapper(object):
    ...
    """
    def __init__(self, batch, mixup_args):
        self.batch = batch
        self.mixup_args = mixup_args
...
def test():
    x = [[1, 2, 3, 4],
         ...
         [9, 0, 3, 3],
         [2, 3, 5]]
    batcher = BucketDataLoader(x, 3, key=lambda z: len(z), shuffle=True, sort_within_batch=True)
    for b in batcher:
        print(b)
